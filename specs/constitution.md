...
- Pacotes seguem `com.wc26.<dominio>`.
- DTOs, comandos, queries e exceções de domínio devem ficar no módulo correto.
- Nomes de módulos usam kebab-case no Gradle e camel/pascal case no código.
- Pacotes Kotlin/Java não podem conter `-` nem `_`; nomes compostos usam `.`.
